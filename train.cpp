...
int main() {
    ...
    loadImagesAndLabels("mnist/labels.csv", "mnist", images, labels, imageSize);

    // Split data into training and validation sets
    int valSize = images.size() / 5;
    ...
    std::vector<std::vector<double>> valImages(images.begin(), images.begin() + valSize);
    std::vector<std::vector<double>> valLabels(labels.begin(), labels.begin() + valSize);
    ...
    mlp.train(trainImages, trainLabels, valImages, valLabels, 0.01, 13);
    mlp.saveWeights("mlp_weights_mnist_60_kernels_30.txt");
